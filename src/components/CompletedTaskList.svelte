<script>
    ...
    function markIncomplete(clickEvent) {
		// find the task that matches the id of passed event
		let task = $completedTasks.find((task) => task.id == clickEvent);

		// update task store array for tasks and completedTasks
        tasks.update((tasks) => [...tasks, task]);
        completedTasks.update((tasks) => [...tasks, task]);

		// update displayed completed tasks on page
        $completedTasks = $completedTasks.filter((task) => task.id !== clickEvent);

		// switch value of completed to false
		task.completed = !task.completed;

		// assignment to update DOM
		$tasks = $tasks;
	}
</script>
...
<ul class="mb-10">
    ...
    {#each $completedTasks as task, i (task.id)}
        <li animate:flip in:fade out:fade class="flex bg-gray-100 rounded-md p-2 pl-4 text-gray-700 mb-2 flex items-center">
            ...
            <input bind:value={task.title} class="bg-gray-100 w-full p-2 line-through focus:outline-none">
